...
| 悬浮窗 | 卡通机器人，眼球追踪鼠标、拖拽、点击弹跳动画，右键智能菜单 |
| 系统托盘 | 代码绘制 AI 图标，右键菜单（设置/插件/通知历史/导出/主题切换） |
| 主题 | 深色/浅色一键切换，所有颜色 DynamicResource 即时生效 |
| 热键 | `Alt+Space` 呼出窗口（可自定义），全局选中文本快捷键 |
...
内置 `IProactivePlugin` 接口支持代码层主动触发（不依赖 AI 工具调用决策），适合天气、新闻等对可靠性要求高的查询。
